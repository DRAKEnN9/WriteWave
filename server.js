...
import serviceAccountKey from "./blog-site-6063d-firebase-adminsdk-htqf5-93b529c6d1.json" assert { type: "json"}
...
server.post("/signup", (req, res) => {
    ...
    // validating he data fron frontend
    ...
    if(!passwordRegex.test(password)){
        return res.status(403).json({"error": "Password should be 6 to 20 character long with a numeric, 1 lowercase, 1 lowercase and 1 uppercase letters"})
    }
...
server.post("/google-auth", async (req, res) => {
    ...
    .then(async (decodedUser) => {
        ...
        if(user){ // login
            ...
            user = new User({
                personal_info: { fullname: name, email, profile_img: picture, username}, // remove profile_img in case we are not getting profile image
                google_auth: true
            })
...
server.post("/change-password", verifyJWT, (req, res) => {
    ...
    if(!passwordRegex.test(currentPassword) || !passwordRegex.test(newPassword)){
        return res.status(403).json({ error: "Password must be between 6 and 20 characters, contain at least one numeric value, 1  uppercase and 1 lowercase letter "})
    }
    ...
    .then((user) => {
        ...
        if(user.google_auth){
            return res.status(403).json({ error: "You can not change the passwod because you logged in through google"})
        }
        ...
        bcrypt.compare(currentPassword, user.personal_info.password, (err, result) => {
            ...
            bcrypt.hash(newPassword, 10, (err, hashed_password) => {
                ...
                .catch(err => {
                    return res.status(500).json({ error: "some error occured while new password new password, please try again later" })
                })
            })
        })
...
server.post("/fetchBlogsFromFollowing", verifyJWT, async (req, res) => {
    ...
    let maxLimit = 5;
        ...
        const user = await User.findById(req.user).populate('account_info.following','personal_info.username')
        ...
        const followedUsers = user.account_info.following.map(followedUser => followedUser._id);
        

        // Fetch blogs from the followed users
        const blogs = await Blog.find({ author: { $in: followedUsers }, draft: false })
            .populate("author", "personal_info.profile_img personal_info.username personal_info.fullname -_id")
            .sort({ "publishedAt": -1 })
            .select("blog_id title des banner activity tags publishedAt -_id")
            .skip((page - 1) * maxLimit)
...
server.post('/following-blog-count', verifyJWT, async (req, res) => {

        const user = await User.findById(req.user).populate('account_info.following', 'personal_info.username')
        const followedUsers = user.account_info.following.map(followedUser => followedUser._id)
        const count = await Blog.countDocuments({ author: { $in: followedUsers }, draft: false })
        .then( count => {
            return res.status(200).json({ totalDocs: count })
        })
        .catch(err=> {
...
server.get("/trending-blogs", (req, res) => {
    ...
    .select("blog_id title des banner publishedAt -_id")
    .limit(5)
    .then(blogs => {
        return res.status(200).json({ blogs })
    })
...
server.post('/create-blog', verifyJWT, async (req, res) => {
    ...
    if(!draft){
        ...
        if(!tags.length || tags.length > 10 ){
            return res.status(403).json({ error: "Provid tags in order to publish the blog, maximum 10"})
        }
    ...
    try {
        if (id) {
            await Blog.findOneAndUpdate({ blog_id }, { title, des, banner, tags, content, draft: draft ? draft : false });
            ...
            if (!draft) {
                // Fetch followers
                const author = await User.findById(authorId).populate('account_info.followers', '_id');
                const followers = author.account_info.followers;

                // Create notifications
                const notifications = followers.map(follower => ({
                    type: "new_blog",
                    blog: blog._id,
                    notification_for: follower._id,
                    user: authorId,
                    seen: false
                }));
...
server.post('/check-following', verifyJWT, async (req, res) => {
    ...
    try {
        ...
        // Check if followId is in the following list of currentUser
        const isFollowing = user.account_info.following.includes(followId);

        return res.status(200).json({ isFollowing, userId });
        }catch(err) {
            console.log(err.message);
            return res.status(500).json({ error: err.message })
        }
    ...
    try {
      ...
      if (followId == userId) {
        ...
        await User.findByIdAndUpdate( followId,{ 
            $push: { "account_info.followers": userId } },
            { new: true }
        )

        await User.findByIdAndUpdate(userId,{ 
            $push: { "account_info.following": followId } },
            { new: true }
        )
        
        const notification = new Notification({
            type: 'follow',
            notification_for: followId,
            user: userId
        });
      ...
      console.error("Error in follow route:", err);  // Log the error for debugging
      return res.status(500).json({ error: err.message });
    }
  });
  ...
  server.post("/unfollow", verifyJWT, async (req, res) => {
    try {
        ...
        await User.findByIdAndUpdate(
            followId,
            { $pull: { "account_info.followers": userId } },
            { new: true }
        );

        await User.findByIdAndUpdate(
            userId,
            { $pull: { "account_info.following": followId } },
            { new: true }
        )
...
server.get("/new-notification", verifyJWT, (req, res) => {
    ...
    .then(result => {
        if(result){
            return res.status(200).json({ new_notification_avilable: true })
        } else{
            return res.status(200).json({ new_notification_avilable: false })
        }
    })
    .catch(err => {
        console.log(err.message);
...
server.post("/notifications", verifyJWT, (req, res) => {
    ...
    .skip(skipDocs)
    .limit(maxLimit)
    .populate("blog", "_id title blog_id")
